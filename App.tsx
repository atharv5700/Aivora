...
const DEFAULT_SETTINGS: AppSettings = {
  theme: 'dark',
  endpoints: [
    { id: 'default-gemini', type: 'gemini', name: 'Google Gemini', apiKey: '', color: '#3b82f6', pricing: { inputRate: 0, outputRate: 0 } },
    { id: 'default-ollama', type: 'ollama', name: 'Local Ollama', baseUrl: 'http://127.0.0.1:11434', color: '#f97316', pricing: { inputRate: 0, outputRate: 0 } }
  ]
};
...
const App: React.FC = () => {
  ...
  // Auto-Save Session
  useEffect(() => {
    if (currentSessionId && columns.length > 0) {
      setSessions(prev => prev.map(s => s.id === currentSessionId ? {
        ...s,
        timestamp: Date.now(),
        columns,
        columnStates: Object.keys(columnStates).reduce((acc, key) => {
           // Only save messages, not error states or thinking states
  ...
  const handleSend = async (overrideInput?: string) => {
    ...
    if (currentSessionId && columns.length > 0 && columnStates[columns[0].id]?.messages.length === 0) {
    ...
    const promises = columns.map(async (col) => {
      ...
      try {
        ...
        for await (const chunk of stream) {
          ...
          let estimatedCost = 0;
          if (endpoint.pricing) {
            estimatedCost = ((lastMetrics.inputTokens / 1000000) * (endpoint.pricing.inputRate || 0)) +
                            ((lastMetrics.outputTokens / 1000000) * (endpoint.pricing.outputRate || 0));
          }
    ...
    <div className="flex h-dvh bg-background text-textMain font-sans overflow-hidden transition-colors duration-300">
      ...
      <div className={`${isSidebarOpen ? 'w-64' : 'w-0'} bg-surface border-r border-border transition-all duration-300 flex flex-col overflow-hidden`}>
         <div className="p-4 flex items-center justify-between border-b border-border">
           ...
           <button onClick={createNewSession} className="p-1.5 hover:bg-surfaceHighlight rounded text-primary"><Plus size={16} /></button>
         </div>
         <div className="flex-1 overflow-y-auto p-2 space-y-1">
           {sessions.map(s => (
             <div key={s.id} onClick={() => loadSession(s)} className={`group flex items-center justify-between p-3 rounded-lg cursor-pointer text-sm transition-colors ${currentSessionId === s.id ? 'bg-surfaceHighlight text-primary' : 'hover:bg-surfaceHighlight text-textMuted'}`}>
               ...
               <button onClick={(e) => deleteSession(e, s.id)} className="opacity-0 group-hover:opacity-100 p-1 hover:text-red-400"><Trash2 size={12} /></button>
             </div>
           ))}
         </div>
      </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="h-14 border-b border-border flex items-center justify-between px-4 bg-background/80 backdrop-blur-md z-30">
          <div className="flex items-center gap-3">
            <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="p-2 hover:bg-surfaceHighlight rounded-md"><SidebarIcon size={18} /></button>
            <div className="flex items-center gap-3">
          ...
          <div className="flex gap-2">
            <button onClick={() => setSettings(p => ({...p, theme: p.theme === 'light' ? 'dark' : 'light'}))} className="p-2 hover:bg-surfaceHighlight rounded-md">{settings.theme === 'light' ? <Moon size={18} /> : <Sun size={18} />}</button>
            <button onClick={() => setIsSettingsOpen(true)} className="p-2 hover:bg-surfaceHighlight rounded-md"><Settings size={18} /></button>
          </div>
        </header>
        ...
        <footer className="p-4 bg-background/80 backdrop-blur-xl border-t border-border z-20">
          <div className="max-w-4xl mx-auto space-y-2">
              ...
              <div className="flex gap-2 overflow-x-auto pb-2">
                {attachments.map((att, i) => (
                  <div key={i} className="relative w-16 h-16 rounded-lg overflow-hidden border border-border group bg-surface flex items-center justify-center">
                    ...
                    <button onClick={() => setAttachments(p => p.filter((_, idx) => idx !== i))} className="absolute inset-0 bg-black/50 flex items-center justify-center opacity-0 group-hover:opacity-100 text-white"><X size={14} /></button>
                  </div>
                ))}
              </div>
            )}
            <div className="relative group flex items-end gap-2 bg-surface border border-border rounded-xl p-2 shadow-2xl">
               ...
               />
               {isThinking ? (
                 <button onClick={handleStop} className="p-3 bg-red-500 hover:bg-red-600 text-white rounded-lg animate-pulse"><StopCircle size={18} /></button>
               ) : (
                 <button onClick={() => handleSend()} disabled={!input.trim() && attachments.length === 0} className="p-3 bg-primary hover:bg-primaryHover text-white rounded-lg disabled:opacity-50"><Send size={18} /></button>
               )}
            </div>
          </div>
        </footer>
        ...
        {isSelectorOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
             <div className="bg-surface w-full max-w-md rounded-xl border border-border shadow-2xl p-6 space-y-5">
                <div className="flex justify-between"><h3 className="font-bold">Add Model</h3><button onClick={() => setIsSelectorOpen(false)}><X size={18} /></button></div>
